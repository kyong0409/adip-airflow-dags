...
이 템플릿은 orderSyncJob을 빌려 쓰지만 시연 연동 대상이 아니므로 dag_id를 별도로 둔다.
주의: 실전 배치 DAG의 dag_id는 Spring Batch job 이름(jobNm)과 정확히 일치해야 한다
(dscore.fw-be AirflowDagAdapter가 jobNm을 dag_id로 사용해 재실행 API를 호출) — README 참고.
"""
...
with DAG(
        ...
        executor_config={
            "pod_override": k8s.V1Pod(
                spec=k8s.V1PodSpec(
                    ...
                    # tolerations=[k8s.V1Toleration(key="batch", operator="Exists")],
                )
            )
        },
    )
    ...
    run_order_sync = KubernetesPodOperator(
        ...
        # env_from=[k8s.V1EnvFromSource(                          # ConfigMap/Secret 통째로 주입할 때
